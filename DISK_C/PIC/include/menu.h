...
void setmenu(Menu *mainmenu, Menu (*submenu)[6], Menu (*thirdmenu)[6]);//设定各级菜单的集成函数
...
int drawsubmenu(Menu (*submenu)[6],Menu mainmenu);//进入次级菜单后，循环检测是否点击了次级菜单的某一项。返回值规则：第a个主菜单的第b个子菜单对应返回值为10*a+b.
...
int drawthirdmenu(Menu (*thirdmenu)[6],int No);//输入参数No表示类型。1是旋转方向选项，2是亮度选项，3是镜像选项，4是滤镜选项，5是程度选项。返回值：1-5是五个程度；6，7是一级，二级变亮；
